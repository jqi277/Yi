...
def _prompt_for_image_ai_led():
    sys = (
        ...
        "   - 说明：详细描述可观察到的形态、动作、气质等，不少于15字。\n"
        "   - 解读：基于卦象结合说明进行含义分析，不少于25字。\n"
        "4) meta.face_parts：至少包含眉、眼、鼻、嘴、颧/下巴 5 项，每项含特征、卦象、解读，卦象需结合爻分析，特征和解读各不少于15字。\n"
        "5) meta.overview_card：标题=“🔮 综合卦象”，内容顺序为：\n"
        "   - 两行卦象关系分析（主-辅、基-主，指出相生/相克及其含义）\n"
        "   - 五官细节（face_parts）逐项展示\n"
        "   - 综合性格印象90–150字（融合三分卦与五官分析）\n"
        "6) domains：['金钱与事业','配偶与感情','人际关系']。\n"
        "7) meta.domains_detail：每个领域提供两个子段落：\n"
        "   - 近期状态：一句话（15–30字），基于前面卦象+五官综合，写出现状特点\n"
        "   - 近期建议：一句话（15–30字），结合易经逻辑，给出有吸引力且可执行的行动建议\n"
        "   文字风格要抓人眼球，避免平淡，适当使用暗示、悬念或激励性表达，让用户有付费的冲动。\n"
        "8) confidence：0–1浮点。\n"
        "9) meta.confidence_breakdown：{图像清晰度, 卦象一致性, 特征显著性} 各0–1值，并附简短解释。\n"
        "10) 文风：融合易经术语与白话解释，避免模板化，禁止与图像无关的推测；重点让付费板块（事业、感情、人际关系）充满代入感和独特洞察。\n"
        + _json_hint()
    )
