...
# 3.8.5-ux: 易经增强版（去重符号、经文提示分行、三才之道）
# - 三分象：清洗“。，/。；”，专业提示【卦·关键词】置前，"经文提示"独立成行
# - 三才之道：主/辅/基三象展开 + 主与辅/基与主生克关系 + 易理综合（不写建议语气）
# - 可信度拆解：meta.confidence_breakdown（图像清晰度/卦象一致性/特征显著性）
# - 顶层 sections 直接采用清洗后的三分象“解读”文本
...
def _prompt_for_image_v372():
    sys = (
      ...
      "然后给出："
      "5) 卦象组合（我们将改写为‘三才之道’）：标题=三象卦名相加（如“艮 + 离 + 兑”），正文 90–150 字。"
...
def _style_by_main_plain(h: str) -> str:
    if h in ("乾","震"): return "整体偏主动"
    if h in ("坤","艮"): return "整体偏稳妥"
    if h in ("离",):     return "整体偏清楚表达"
    if h in ("兑",):     return "整体偏亲和"
    if h in ("巽",):     return "整体偏协调"
    if h in ("坎",):     return "整体偏谨慎"
    return "整体风格平衡"
...
def _pair_relation_phrase(a_ele: str, b_ele: str) -> (str, str):
    ...
    if SHENG.get(b_ele) == a_ele: return f"{b_ele}生{a_ele}", "相生"   # other -> main
...
def _synthesize_combo(hexes: List[str]) -> (str, str):
    ...
    content = "\n".join(lines)
    card_title = f"三才之道（{title}）" if title else "三才之道"
...
def _coerce_output(data: Dict[str,Any]) -> Dict[str,Any]:
    ...
    # 可信度拆解（固定权重，可后续接入模型评分）
    cb = meta.get("confidence_breakdown") or {
        "图像清晰度": 0.30,
        "卦象一致性": 0.40,
        "特征显著性": 0.30,
    }
    ...
    meta["combo_title"] = card_title.replace("三才之道（", "").rstrip("）")
    ...
    out["archetype"] = _clean_text(out.get("archetype",""))

    # 领域：状态与建议（建议仍保留在 meta 下供前端可选使用）
